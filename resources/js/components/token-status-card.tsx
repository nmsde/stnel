...
import { CheckCircle, AlertTriangle, XCircle, RefreshCw, Clock, Shield, ExternalLink } from 'lucide-react';
    ...
    if (showRenewalFlow) {
        return (
            <div className={`space-y-6 ${className}`}>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    Update your Cloudflare API token for {organisation.name}
                                </CardDescription>
                            </div>
                            <Button 
                                variant="ghost" 
                ...
                <CloudflareTokenSetup 
                    organisationId={organisation.id}
                    onTokenValidated={handleTokenRenewed}
        ...
        <Card className={className}>
            ...
            <CardContent className="space-y-4">
                <div className="flex items-start gap-3">
                    <StatusIcon className={`h-5 w-5 mt-0.5 ${tokenStatus.color}`} />
                ...
                {organisation.token_permissions && Object.keys(organisation.token_permissions).length > 0 && (
                    <div className="pt-2 border-t">
                        <p className="text-xs font-medium text-muted-foreground mb-2">Token Permissions:</p>
                        <div className="flex flex-wrap gap-1">
                            {Object.entries(organisation.token_permissions).map(([key, permission]: [string, any]) => (
                                <Badge 
                                    key={key} 
                                    variant={permission.granted ? "secondary" : "destructive"}
                                    className="text-xs"
                                >
